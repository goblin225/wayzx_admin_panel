                      ...
                      <DropdownMenuTrigger asChild>
                        <div className={`${booking.color} text-white text-xs px-2 py-1 rounded mt-1 cursor-pointer`}>
                          ...
                          <p className="text-[10px]">
                            {`${format(parseISO(booking.start_date), 'MMM d, yyyy h:mm a')} - ${format(parseISO(booking.end_date), 'MMM d, yyyy h:mm a')}`}
                          </p>
                        </div>
                      </DropdownMenuTrigger>
                          ...
                          <div className="w-1/2 p-4 border-r">
                            <div className={`${booking.color} text-white px-3 py-2 rounded mb-3`}>
                              ...
                              <p className="text-xs opacity-80">
                                {format(parseISO(booking.startDate), "MMM d, h:mm a")} - {booking.end}
                              </p>
                            </div>
                            <div className="space-y-2 text-sm">
                              ...
                              <div className="flex justify-between"><span className="font-medium text-gray-600">Location:</span><span>{booking.location}</span></div>
                              <div className="flex justify-between"><span className="font-medium text-gray-600">Room:</span><span>{booking.roomId?.name}</span></div>
                              <div className="flex justify-between"><span className="font-medium text-gray-600">Space Title:</span><span>{booking.title}</span></div>
                              ...
                              <div className="flex justify-between"><span className="font-medium text-gray-600">Duration:</span><span>{booking.start}, {booking.end}</span></div>
                            </div>
                          </div>
                          <div className="w-1/2 p-4">
                            <div className="space-y-2 text-sm">
                              <div className="flex justify-between"><span className="font-medium text-gray-600">Name:</span><span>{booking.name}</span></div>
                              <div className="flex justify-between"><span className="font-medium text-gray-600">Email:</span><span>{booking.email}</span></div>
                              <div className="flex justify-between"><span className="font-medium text-gray-600">Guests:</span><span>{booking.guests}</span></div>
                              <div className="flex justify-between"><span className="font-medium text-gray-600">Price/Hour:</span><span>₹{booking.pricePerHour}</span></div>
